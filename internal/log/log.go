// log --> which manages the list of segments
package log
...
// to append writes to.
// the Dir is where we store the segments.
type Log struct {
	mu            sync.RWMutex
	Dir           string
	Config        Config
...
func (l *Log) setup() error {
	...
	for i := 0; i < len(baseOffset); i++ {
		...
		// baseOffset contains duplicates for index and store so we skip the dup
		i++
	}
	if l.segments == nil {
		if err = l.newSegment(
...
// Close, iterates over the segments adn closes them.
...
// Truncate removes all segments whose highest offset
...
// Reader returns a io.MultiReader type to concatenate
// the segment's stores.
...
// newSegment(off int64) creates a new segment,
// appends that segment to the log’s slice of segments,
// and makes the new segment the active segment
// so that subsequent append calls write to it.
